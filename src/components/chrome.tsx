...
export function Header({ session }: { session: Session | null }) {
  ...
  return <><motion.header className={`site-header ${scrolled ? "scrolled" : ""}`} layout transition={{ duration: reduced ? 0 : .28, ease: [.2, .75, .25, 1] }}>
    ...
    <div className="category-bar"><nav aria-label="Categorias"><button className="all-categories" onClick={() => setMenu((open) => !open)} aria-expanded={menu}><span>Todos os produtos</span><motion.span animate={{ rotate: menu ? 180 : 0 }}><ChevronDown aria-hidden="true" /></motion.span></button><Link href="/produtos?categoria=caes">Cães</Link><Link href="/produtos?categoria=gatos">Gatos</Link><Link href="/produtos?tipo=racao">Rações</Link><Link href="/produtos?tipo=petisco">Petiscos</Link><Link href="/banho-e-tosa">Banho & tosa</Link><Link href="/atendimento">Atendimento</Link><Link href="/loja">A Rottava</Link></nav></div>
    <AnimatePresence>{menu && <motion.div className="mega-menu" initial={reduced ? false : { opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={reduced ? undefined : { opacity: 0, y: -8 }} transition={{ duration: .2 }}><div><span>COMPRE POR ROTINA</span><Link href="/produtos?tipo=racao" onClick={() => setMenu(false)}>Alimentação diária <ArrowUpRight aria-hidden="true" /></Link><Link href="/produtos?tipo=sache" onClick={() => setMenu(false)}>Alimento úmido <ArrowUpRight aria-hidden="true" /></Link><Link href="/produtos?tipo=petisco" onClick={() => setMenu(false)}>Petiscos <ArrowUpRight aria-hidden="true" /></Link></div><div><span>ESCOLHA PELO PET</span><Link href="/produtos?categoria=caes" onClick={() => setMenu(false)}>Cães</Link><Link href="/produtos?categoria=gatos" onClick={() => setMenu(false)}>Gatos</Link><Link href="/banho-e-tosa" onClick={() => setMenu(false)}>Banho & tosa</Link></div><Link className="mega-feature" href="/" onClick={() => setMenu(false)}><small>COMPRA ASSISTIDA</small><b>Responda quatro perguntas e compare opções.</b><ArrowUpRight aria-hidden="true" /></Link></motion.div>}</AnimatePresence>
...
export function Footer() {
  return <footer className="site-footer"><div className="footer-top"><div><span>ATENDIMENTO ROTTAVA</span><h2>Precisa de ajuda para escolher?</h2></div><Link className="footer-cta" href="/atendimento">Falar com a equipe <ArrowUpRight /></Link></div><div className="footer-grid">
    ...
    <div><h3>Compre</h3><Link href="/produtos">Todos os produtos</Link><Link href="/produtos?categoria=caes">Para cães</Link><Link href="/produtos?categoria=gatos">Para gatos</Link><Link href="/favoritos">Favoritos</Link></div>
