...
type CartValue = { items: CartItem[]; count: number; subtotal: number; isOpen: boolean; open: () => void; close: () => void; add: (item: Omit<CartItem, "quantity">, options?: { open?: boolean }) => void; update: (id: string, quantity: number) => void; clear: () => void };
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<CartValue>(() => ({
    ...
    add(item, options) { setItems((all) => { const found = all.find((i) => i.variantId === item.variantId); return found ? all.map((i) => i.variantId === item.variantId ? { ...i, quantity: Math.min(99, i.quantity + 1) } : i) : [...all, { ...item, quantity: 1 }]; }); if (options?.open) setIsOpen(true); },
