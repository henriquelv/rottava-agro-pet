...
function QuickAdd({ product, variant, label = false }: { product: Product; variant: Product["variants"][number]; label?: boolean }) {
  ...
  function commit(selected: Product["variants"][number]) {
    ...
    setOpen(false); setDone(true); toast.success(`${product.name} entrou no carrinho`); window.setTimeout(() => setDone(false), 1400);
  }
...
export function ProductBuy({ product }: { product: Product }) {
  ...
  function addSelected(open = true) { add({ variantId: variant!.id, productSlug: product.slug, name: product.name, variant: variant!.label, priceCents: variant!.price_cents, imageUrl: product.images[0] || product.image_url }, { open, quantity }); setDone(true); toast.success(`${quantity} ${quantity === 1 ? "item adicionado" : "itens adicionados"} ao carrinho`); window.setTimeout(() => setDone(false), 1600); }
  ...
  return <div className="buy-box"><fieldset><legend>Selecione a opção</legend><div className="variant-grid">{product.variants.map((v) => <button type="button" aria-pressed={selected === v.id} className={selected === v.id ? "selected" : ""} key={v.id} onClick={() => setSelected(v.id)}><span>{v.label}</span><b>{money(v.price_cents)}</b></button>)}</div></fieldset>
    ...
    <div className="pdp-secondary-actions"><button className="button secondary" disabled={variant.stock_quantity === 0} onClick={buyNow}>Comprar agora</button><button className={`pdp-favorite ${favorites.has(product.id) ? "active" : ""}`} onClick={() => { favorites.toggle(product.id); toast(favorites.has(product.id) ? "Removido dos favoritos" : "Adicionado aos favoritos"); }} aria-label={favorites.has(product.id) ? "Remover dos favoritos" : "Adicionar aos favoritos"}><Heart fill={favorites.has(product.id) ? "currentColor" : "none"} aria-hidden="true" />{favorites.has(product.id) ? "Favoritado" : "Favoritar"}</button></div>
