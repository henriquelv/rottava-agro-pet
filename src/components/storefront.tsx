...
import { ArrowRight, Minus, Plus, ShoppingBag, Trash2, Check, LoaderCircle, Send } from "lucide-react";
...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card">
    <Link href={`/produto/${product.slug}`} className="product-image">{product.image_url ? <Image unoptimized src={product.image_url} alt="" fill sizes="(max-width: 700px) 50vw, 25vw" /> : <span className="image-placeholder">ROTTAVA</span>}</Link>
...
export function CheckoutView() {
  const cart = useCart(); const router = useRouter(); const [mode, setMode] = useState<"pickup" | "delivery">("pickup"); const [payment, setPayment] = useState<"in_person" | "pix" | "card">("in_person"); const [state, setState] = useState<"idle" | "loading" | "error">("idle"); const [message, setMessage] = useState("");
  async function submit() {
    setState("loading"); setMessage("");
    const response = await fetch("/api/orders", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ items: cart.items.map((i) => ({ variantId: i.variantId, quantity: i.quantity })), fulfillmentMode: mode, paymentMode: payment, idempotencyKey: crypto.randomUUID() }) });
