...
import { ArrowRight, CalendarDays, CheckCircle2, Clock3, MapPin, PawPrint, ShoppingBag, Sparkles, Truck } from "@/components/icons";
...
export default async function RouterPage({ params, searchParams }: Props) {
  ...
  if (path === "/") return <Home />;
  if (path === "/produtos") return <Catalog search={typeof query.busca === "string" ? query.busca : ""} category={typeof query.categoria === "string" ? query.categoria : ""} brand={typeof query.marca === "string" ? query.marca : ""} sort={typeof query.ordem === "string" ? query.ordem : ""} minPrice={typeof query.precoMin === "string" ? query.precoMin : ""} maxPrice={typeof query.precoMax === "string" ? query.precoMax : ""} available={query.disponivel === "1"} />;
...
async function Home() {
  ...
  return <>
    ...
    <Reveal><section className="category-journal"><header><span className="eyebrow">ESCOLHA PELO UNIVERSO</span><h2>Uma casa viva tem<br /><em>muitas rotinas.</em></h2><p>Entre pelo contexto e encontre somente o que faz sentido para o seu dia.</p></header><div className="category-collage">
      ...
      <Link className="category-photo category-photo-garden" href="/produtos?categoria=casa-jardim"><Image src="/images/demo/jardim-editorial.webp" fill sizes="(max-width: 760px) 50vw, 25vw" alt="Plantas e objetos para jardim" /><span><small>03 · VERDE</small><b>Casa & jardim</b></span></Link>
...
async function Catalog({ search, category, brand, sort, minPrice, maxPrice, available }: { search: string; category: string; brand: string; sort: string; minPrice: string; maxPrice: string; available: boolean }) {
  const allProducts = await listProducts(); const products = await listProducts(search, { category, brand, sort, minPrice: Number(minPrice) || undefined, maxPrice: Number(maxPrice) || undefined, available });
