...
import { ArrowRight, CalendarDays, CheckCircle2, Clock3, Leaf, MapPin, PackageCheck, PawPrint, ShieldCheck, ShoppingBag, Sparkles, Truck, BadgeCheck, Headphones, Sprout, Dog, Cat } from "lucide-react";
...
async function Home() {
  ...
  return <>
    <section className="home-hero"><Image src="/images/hero-rottava.png" fill priority sizes="100vw" alt="Cão e gato descansando juntos em uma varanda com jardim" /><div className="hero-shade" /><div className="glyph-portal" aria-hidden="true">{"ROTTAVA".split("").map((letter, index) => <span key={index}>{letter}</span>)}</div><div className="home-copy"><span className="eyebrow">PET · CASA · JARDIM · CAÇADOR</span><h1>Um mundo inteiro de <em>cuidado</em>, logo ali.</h1><p>Escolhas para os seus companheiros e para a vida que vocês constroem juntos.</p><div className="hero-actions"><Link className="button light" href="/produtos">Explorar produtos <ArrowRight /></Link><Link className="button outline-light" href="/banho-e-tosa">Conhecer cuidados</Link></div></div><div className="hero-caption"><span>DESDE PERTO</span><p>Presença que acolhe.<br />Cuidado que acompanha.</p></div>
    </section>
    <section className="trust-strip"><span><ShieldCheck /> Compra segura e identificada</span><span><PackageCheck /> Retirada após confirmação</span><span><Headphones /> Atendimento humano quando precisar</span><span><PawPrint /> Cuidado para cada fase</span></section>
    <Reveal><section className="category-showcase"><div className="section-heading"><div><span className="eyebrow">ENCONTRE O SEU UNIVERSO</span><h2>Para cada canto da sua rotina.</h2></div><p>Uma curadoria próxima, prática e cuidadosa — do pote ao quintal.</p></div><div className="category-mosaic"><Link className="category-pet" href="/produtos?categoria=pet"><Dog /><span>01 / COMPANHEIROS</span><h3>Para cães</h3><p>Alimentação, passeio e conforto.</p><ArrowRight /></Link><Link className="category-cat" href="/produtos?categoria=pet"><Cat /><span>02 / FELINOS</span><h3>Para gatos</h3><p>Rotina, diversão e bem-estar.</p><ArrowRight /></Link><Link className="category-garden" href="/produtos?categoria=casa-jardim"><Sprout /><span>03 / CASA VIVA</span><h3>Casa & jardim</h3><p>Do verde de dentro ao quintal.</p><ArrowRight /></Link><Link className="category-care" href="/produtos?categoria=cuidados"><Sparkles /><span>04 / CUIDADOS</span><h3>Higiene</h3><p>Pequenos rituais, grande diferença.</p><ArrowRight /></Link></div></section></Reveal>
    <section className="featured"><Reveal><div className="section-heading"><div><span className="eyebrow">MAIS PEDIDOS</span><h2>Escolhas que já fazem parte da casa.</h2></div><Link className="text-link" href="/produtos">Ver catálogo <ArrowRight /></Link></div></Reveal>{products.length ? <div className="product-rail">{products.slice(0, 6).map((p) => <ProductCard key={p.id} product={p} />)}</div> : <EmptyState title="A vitrine está sendo preparada" text="O catálogo real ainda não foi conectado ou publicado." action={{ href: "/atendimento", label: "Falar com a loja" }} />}</section>
    <section className="brand-marquee" aria-label="Marcas no catálogo"><span>CURADORIA ROTTAVA</span><div>{["Rottava Essencial", "Casa Animal", "Pata Livre", "Verde Perto", "Quintal Bom", "Rottava Essencial"].map((brand, index) => <b key={`${brand}-${index}`}>{brand}<i>✦</i></b>)}</div></section>
    <Reveal><section className="editorial-story"><div className="story-image"><Image src="/images/hero-rottava.png" fill sizes="50vw" alt="Vida com pets e jardim" /></div><div><span className="eyebrow">VIDA BOA É VIDA COMPARTILHADA</span><h2>A casa fica mais casa com eles por perto.</h2><p>Por isso reunimos escolhas honestas para alimentar, cuidar, brincar, plantar e viver. Sem excesso. Com atenção ao que realmente acompanha a rotina.</p><Link className="text-link" href="/loja">Conheça a Rottava <ArrowRight /></Link></div></section></Reveal>
    <section className="benefit-grid"><article><BadgeCheck /><b>Curadoria próxima</b><p>Produtos organizados para decisões simples e claras.</p></article><article><ShieldCheck /><b>Compra protegida</b><p>Identidade, preço e disponibilidade revalidados.</p></article><article><Headphones /><b>Atendimento de verdade</b><p>O chat encaminha para a equipe quando necessário.</p></article><article><Truck /><b>Receba do seu jeito</b><p>Retirada disponível após a confirmação da loja.</p></article></section>
...
async function Catalog({ search, category, brand, sort }: { search: string; category: string; brand: string; sort: string }) {
  ...
  return <div className="page-wrap catalog-page"><PageHero eyebrow="CATÁLOGO ROTTAVA" title={search ? `Resultados para “${search}”` : "Boas escolhas começam aqui."} text="Produtos para a vida com pets, casa e jardim — com variações claras e disponibilidade confirmada antes do pedido." actions={<form action="/produtos" className="catalog-search"><input name="busca" defaultValue={search} placeholder="Buscar por produto ou marca" /><button className="button primary">Buscar</button></form>} />{demoMode && <p className="demo-caption">Ambiente demonstrativo: nomes, marcas, preços e estoques servem para validar toda a experiência.</p>}<div className="catalog-layout"><CatalogFilters brands={brands} total={products.length} />{products.length ? <div className="product-grid catalog-grid">{products.map((p) => <ProductCard key={p.id} product={p} />)}</div> : <div className="catalog-empty"><EmptyState title="Nenhum produto encontrado" text="Tente combinar outros filtros ou limpe a busca." action={{ href: "/produtos", label: "Limpar filtros" }} /></div>}</div></div>;
}
...
async function ProductPage({ slug }: { slug: string }) {
  ...
  const schema = { "@context": "https://schema.org", "@type": "Product", name: product.name, description: product.description, sku: product.variants[0]?.sku, brand: product.brand ? { "@type": "Brand", name: product.brand } : undefined, offers: product.min_price_cents == null ? undefined : { "@type": "Offer", priceCurrency: "BRL", price: (product.min_price_cents / 100).toFixed(2), availability: "https://schema.org/InStock" } };
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema).replaceAll("<", "\\u003c") }} /><div className="product-breadcrumbs"><Link href="/">Início</Link><span>/</span><Link href="/produtos">Produtos</Link><span>/</span><span>{product.name}</span></div><div className="product-page"><div className="product-gallery">{["principal", "detalhe", "rotina"].map((view, index) => <div className={`gallery-frame frame-${index}`} key={view}>{product.image_url && index === 0 ? <Image unoptimized src={product.image_url} alt={product.name} fill sizes="(max-width: 800px) 100vw, 45vw" /> : <div className="gallery-art"><span>{product.category_name === "Casa & Jardim" ? <Leaf /> : product.category_name === "Cuidados" ? <Sparkles /> : <PawPrint />}</span><small>{view}</small><b>{product.brand}</b></div>}</div>)}</div><div className="product-info"><span className="eyebrow">{product.category_name || "PRODUTO"}{product.brand && ` · ${product.brand}`}</span><div className="product-rating"><span>★★★★★</span><b>{product.rating?.toFixed(1) || "Novo"}</b><small>{product.reviews_count ? `${product.reviews_count} avaliações demonstrativas` : "Sem avaliações"}</small></div><h1>{product.name}</h1><p>{product.description || "As informações detalhadas deste produto ainda estão sendo revisadas pela loja."}</p><ProductBuy product={product} /><div className="product-accordions"><details open><summary>Sobre este produto</summary><p>Descrição demonstrativa para validar a leitura da página. Conteúdo técnico e comercial definitivo virá do catálogo homologado.</p></details><details><summary>Compra e disponibilidade</summary><p>O carrinho não reserva estoque. A loja confirma preço, variante e disponibilidade na criação do pedido.</p></details><details><summary>Entrega e retirada</summary><p>A retirada depende de confirmação. A entrega só é oferecida após uma cotação válida para o endereço.</p></details></div></div></div>{related.length > 0 && <section className="related-products"><div className="section-heading"><div><span className="eyebrow">COMPLETE A ROTINA</span><h2>Também pode fazer sentido.</h2></div><Link className="text-link" href="/produtos">Ver mais <ArrowRight /></Link></div><div className="product-grid">{related.map((item) => <ProductCard key={item.id} product={item} />)}</div></section>}</>;
}
